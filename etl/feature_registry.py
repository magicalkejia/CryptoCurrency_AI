...
FEATURE_DEFINITIONS: dict[str, dict[str, str]] = {
    ...
    "cvd": {"group": "flow", "definition": "Kline-level cumulative volume delta from USD-M futures taker flow.", "calculation": "cumsum(cvd_delta) per symbol, where cvd_delta = taker_buy_vol - taker_sell_vol.", "source": "processed/flow/cvd.parquet", "usage": "Cumulative aggressive buy/sell pressure. This is kline-level taker CVD, not tick-level true CVD."},
    "cvd_delta": {"group": "flow", "definition": "Taker buy volume minus taker sell volume over the processed CVD bar.", "calculation": "taker_buy_vol - (volume - taker_buy_vol), resampled to the CVD timeframe.", "source": "processed/flow/cvd.parquet", "usage": "Incremental aggressive order-flow pressure."},
    "cvd_delta_chg_1": {"group": "flow", "definition": "Change in CVD delta versus previous processed bar.", "calculation": "cvd_delta.diff() per symbol.", "source": "processed/flow/cvd.parquet", "usage": "Acceleration/deceleration of aggressive flow."},
    "cvd_delta_z_30": {"group": "flow", "definition": "Rolling z-score of CVD delta over 30 processed bars.", "calculation": "zscore(cvd_delta, rolling window=30 bars) per symbol.", "source": "processed/flow/cvd.parquet", "usage": "Abnormal aggressive flow pressure."},
    "perp_premium": {"group": "spot_perp_basis", "definition": "Perpetual futures close relative to spot close.", "calculation": "perp_close / spot_close - 1.", "source": "processed/derivatives/spot_perp_basis.parquet", "usage": "Positive values mean the perp trades above spot, often reflecting stronger derivatives demand."},
    "spot_premium": {"group": "spot_perp_basis", "definition": "Spot close relative to perpetual futures close.", "calculation": "spot_close / perp_close - 1.", "source": "processed/derivatives/spot_perp_basis.parquet", "usage": "Positive values mean spot trades above perp, often reflecting stronger spot demand."},
    "perp_premium_bps": {"group": "spot_perp_basis", "definition": "Perpetual premium in basis points.", "calculation": "(perp_close / spot_close - 1) * 10000.", "source": "processed/derivatives/spot_perp_basis.parquet", "usage": "Human-readable spot/perp basis magnitude."},
    "perp_premium_chg_1": {"group": "spot_perp_basis", "definition": "One-bar change in perpetual premium.", "calculation": "perp_premium.diff() per symbol.", "source": "processed/derivatives/spot_perp_basis.parquet", "usage": "Expansion/contraction of derivatives premium."},
    "perp_premium_z_30": {"group": "spot_perp_basis", "definition": "Rolling z-score of perpetual premium over 30 processed bars.", "calculation": "zscore(perp_premium, rolling window=30 bars) per symbol.", "source": "processed/derivatives/spot_perp_basis.parquet", "usage": "Abnormal spot/perp dislocation."},
